...
Instead we can use the **In-System Programming** (ISP), also known as **In-Circuit Serial Programming** (ICSP)!
...
As you see from the image below we need some soldering to extend the headers' legs. We also need pliers to gently bend them in order to and make them perfectly match the rails of the breadboard.
...
Now it's time to upload a program usign the programmer and there are at least two way we can achieve this:
...
Because our ATmega328p came with the bootloader preinstalled we weren't able to test it but, according to the Arduino instructions "_Upload Using Programmer_" method, if yours is a fresh factory micro-controller, you must first burn a bootloader to it. We think this is needed in order to properly set the mcu fuses so to upload programs.
{: class="note"}
...
After adding this rule, you need to reboot the computer in order for this modification to take effect, but after doing this this, you will be able to use the particular usb device without the need of _root_ permissions.

### Usign the terminal commands: the hard way!
...
Even if you are not usign the Arduino IDE, don't forget to create the **udev rule** file in order to have the right permission to use the USBtiny device! See the section above for more details.
{: class="dashed"}
...
First let's write a simple _C_ program to replicate the legendary "_blink_" sketch of Arduino:
...
* ```avr/io.h```: this header file includes the apropriate IO definitions for the device that will be specified by the ```-mmcu=``` compiler command-line switch (see below) (here's link to the [documentation](http://www.nongnu.org/avr-libc/user-manual/group__avr__io.html));
...
Going on we enter the ```main```: here we set the local variable ```interval``` to 1000, because we want 1 second to be the time during which our LED will stay on and off.
...
```
This instruction tells ```avr-gcc``` to create an _object code file_ - ```filename.o``` - named after the program source code ```filename.c```. It also tells the compiler the type of mcu we need to build the program for (**mmcu** option).
...
We can use the following command in order to copy the file from the computer to the mcu memory using the USBtinyISP hardware programmer:
```
...
To fully understand how ```avrdure``` works and what all these arguments stand for, we recommend you to read the command manual (```man avrdude```), but it'is worth here to have a little recap:
...
2. We can create a new file on our home directory an place a line inside it telling that USBtinyISP is our default programmer. This way we can omit the ```-c``` option. The file we create is named ```.avrduderc``` and it contains this single line:
```
...
This experiment opens up a lot of other possibilities: we can for example gather our entire workflow inside a single ```makefile``` to do everything quicker; This will be extremely useful if we think of projects with tens or maybe hundreds of micro-controllers to program.
